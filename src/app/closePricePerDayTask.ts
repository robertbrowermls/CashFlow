...
export async function runClosePricePerDayTask(config: RuntimeConfig): Promise<void> {
  logger.info('Close Price Per Day task:', 'Started.');
  ...
  try {
    ...
    if (userProfile.account.status !== "active") {
      ...
      logger.error('Close Price Per Day task:', error);
      ...
      for (const position of positions) {
        ...
        if (!dbTrade) {
          ...
          try {
            ...
            logger.debug('Close Price Per Day task:', `current short put: ${JSON.stringify(shortPut)}`);
            logger.debug('Close Price Per Day task:', `current long put: ${JSON.stringify(longPut)}`);
            logger.debug('Close Price Per Day task:', `prevCredit: ${openCredit}, prevDaysToExpiration: ${openDaysToExpiration}, prevPricePerDay: ${openPricePerDay}`);
            logger.debug('Close Price Per Day task:', `currentCredit: ${currentCredit}, currentDaysToExpiration: ${currentDaysToExpiration}, currentPricePerDay: ${currentPricePerDay}`);
            ...
            // Exit the position if the price per day is too low.
            if (currentPricePerDayTooLow && !tradesClosed[dbTrade.underlying]) {
              ...
              const exitedMessage = `Successfully exited ${dbTrade.underlying} position because the price per day was too low.`;
              logger.info('Close Price Per Day task:', exitedMessage);
            }
          }
          catch (error) {
    ...
    if (positionsNotFoundInDb.length > 0) {
      const notFoundMessage = `Positions not found in DB: ${positionsNotFoundInDb.join(', ')}. The app cannot determine if it should exit these positions. Consider manually closing these positions.`;
      logger.info('Close Price Per Day task:', notFoundMessage);
    ...
    if (positionsOutOfTheMoney.length > 0) {
      ...
      logger.info('Close Price Per Day task:', outOfTheMoneyMessage);
    }
    ...
    if (closedTrades.length !== prevDbClosedTrades.length) {
      reportData.title = 'Closed Trades (price per day too low)';
    ...
    logger.info('Close Price Per Day task:', 'Succeeded.');
